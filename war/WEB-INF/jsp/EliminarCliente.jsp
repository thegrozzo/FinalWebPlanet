...
<table>
<tr>		
  <th><label>Clientes Registrados:</label></th>
  <th><select name="dni">
  				    <%
				    for (int idx = 0; idx < us.size(); idx++) {
				     Usuarios u = (Usuarios)us.get(idx);
				    %>
				    <option value="<%= u.getDNI() %>"><%= "DNI:"+u.getDNI()+"-USUARIO:"+u.getUser()+"-NOMBRE:"+u.getNombre()+" "+u.getApellido() %></option>
				     <%}%>
				  </select></th>		
